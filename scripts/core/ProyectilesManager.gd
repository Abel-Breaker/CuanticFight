...
func spawn_proyectile(owner_id: int, type : ProyectileType, global_position: Vector2, velocity : Vector2, layer: int, mask: int, gravity_scale: float, dealing_dmg: int):
	...
	proyectile.setup(owner_id, dealing_dmg, layer, mask)
